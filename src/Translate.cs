...
{
    public static string TranslateFrom(string DNA, string From)
    {
        ...
        for (int i = 0; i < DNA.Length; i++)    //Loop through sequence until start codon (atg)
        {
            ...
            {
                string Output = "";
                (i, Output) = TranslateCodons(OldCode, i, new CodonEncoding(From), GIL.Program.CurrentEncoding);    //jump to letter after stop codon
    ...
    static (int, string) TranslateCodons(string DNA, int Pos, CodonEncoding From, CodonEncoding To)    //translate protein sequence
    {
        ...
        for (i = Pos; i < DNA.Length - 2; i += 3)    //DNA.Length - 3 fails to translate the last codon
        {
            ...
            {
                Output.Append(To.GetLetter(c.Item1, c.Item2));
                continue;
            }
            Output.Append(To.GetLetter(c.Item1, c.Item2));    //Otherwise, translate and break out of loop
            i += 2;
            break;
        }
    ...
    public static string TranslateTo(string DNA, string Target)
    {
        string OldCode = DNA.ToLower();    //Convert sequence to lower case
        StringBuilder NewCode = new StringBuilder("", DNA.Length);    //Since we know the max length, we use a StringBuilder
        ...
        for (int i = 0; i < DNA.Length; i++)    //Loop through sequence until start codon (atg)
        {
            ...
            {
                string Output = "";
                (i, Output) = TranslateCodons(OldCode, i, GIL.Program.CurrentEncoding, new CodonEncoding(Target));    //jump to letter after stop codon
                NewCode.Append(Output);
                continue;
            }
            NewCode.Append(OldCode[i]);    //If no start codon, just append the sequence to the output
